...
class Category {
    ...
    static async getCategoryById(categoryId) {
        const result = await query('SELECT * FROM product_category WHERE id = ?', [categoryId]);
        return result[0];
    }
    ...
    async update(id) {
      ...
      const updateableProperties = ['name', 'description', 'is_active', 'image_url', 'priority'];

      // Filter properties that have changed and map them to SQL update format
      const setClause = Object.entries(this)
        .filter(([key, value]) => updateableProperties.includes(key) && value !== undefined)
        .map(([key]) => `${key} = ?`)
        .join(', ');
    ...
    async save() {
        await query('INSERT INTO product_category (name, description, is_active, image_url, priority) VALUES (?, ?, ?, ?, ?)',
            [this.name, this.description, this.isActive, this.imageUrl, this.priority]);
    }
}
